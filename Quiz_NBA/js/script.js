const STORE = {
 questions: {
  nba: [
   {
    question: "Quando a NBA foi fundada?",
    code: ``,
    answers: [
     "1946",
     "1950",
     "1955",
     "1960"
    ],
    correctAnswer: "1946"
   },{
    question: "Qual time ganhou o primeiro campeonato da NBA?",
    code: ``,
    answers: [
     "Boston Celtics",
     "Philadelphia Warriors",
     "Mineapolis Lakers",
     "New York Knicks"
    ],
    correctAnswer: "Philadelphia Warriors"
   },{
    question: "Quem é o maior pontuador de todos os tempos da história da NBA?",
    code: ``,
    answers: [
     "LeBron James",
     "Michael Jordan",
     "Kareem Abdul-Jabbar",
     "Kobe Bryant"
    ],
    correctAnswer: "LeBron James"
   },{
    question: "Quantos times estavam na NBA quando ela foi criada?",
    code: ``,
    answers: [
     "8",
     "11",
     "22",
     "16"
    ],
    correctAnswer: "11"
   },{
    question: "Quem foi o primeiro jogador a marcar 100 pontos em um único jogo?",
    code: ``,
    answers: [
     "Wilt Chamberlain",
     "Michael Jordan",
     "Kobe Bryant",
     "Shaquille O'Neal"
    ],
    correctAnswer: "Wilt Chamberlain"
   }
  ],
  nba2: [
   {
    question: "Quem foi uma das primeiras estrelas da NBA?",
    code: ``,
    answers: [
     "George Mikan",
     "Bob Cousy",
     "Bill Russel",
     "Wil Chamberlain"
    ],
    correctAnswer: "George Mikan"
   },{
    question: "Quem foi o primeiro treinador afro-americano na NBA?",
    code: ``,
    answers: [
     "Bill Russel",
     "Lenny Wilkens",
     "Al Attles",
     "Chuck Cooper"
    ],
    correctAnswer: "Bill Russel"
   },{
    question: "Qual time detém o recorde da mais longa sequência de vitórias na história da NBA?",
    code: ``,
    answers: [
     "Chicago Bulls",
     "Los Angeles Lakers",
     "Boston Celtics",
     "Miami Heat"
    ],
    correctAnswer: "Los Angeles Lakers"
   },{
    question: "Quando foi introduzida a linha de três pontos na NBA?",
    code: ``,
    answers: [
     "1967",
     "1970",
     "1979",
     "1984"
    ],
    correctAnswer: "1979"
   },{
    question: "Qual jogador era conhecido como “O Logotipo” da NBA?",
    code: ``,
    answers: [
     "Jerry West",
     "Larry Bird",
     "Magic Johnson",
     "Bill Russel"
    ],
    correctAnswer: "Jerry West"
   }
  ],
  nba3: [
   {
    question: "Quem foi o jogador mais jovem a ser convocado para a NBA?",
    code: ``,
    answers: [
     "LeBron James",
     "Kobe Bryant",
     "Kevin Garnett",
     "Andrew Bynum"
    ],
    correctAnswer: "Andrew Bynum"
   },{
    question: "Qual jogador tem mais assistências na carreira na NBA?",
    code: ``,
    answers: [
     "Steve Nash",
     "John Stockton",
     "Magic Johnson",
     "Jason Kidd"
    ],
    correctAnswer: "John Stockton"
   },{
    question: "Qual time convocou Kobe Bryant?",
    code: ``,
    answers: [
     "Los Angeles Lakers",
     "Charlotte Hornets",
     "Philadelphia 76ers",
     "Golden State Warriors"
    ],
    correctAnswer: "Charlotte Hornets"
   },{
    question: "Em que ano a NBA se fundiu com a ABA?",
    code: ``,
    answers: [
     "1970",
     "1976",
     "1980",
     "1984"
    ],
    correctAnswer: "1976"
   },{
    question: "Quem foi o primeiro jogador europeu a ganhar o prêmio NBA MVP?",
    code: ``,
    answers: [
     "Dirk Nowitzki",
     "Pau Gasol",
     "Giannis Antetokounmpo",
     "Tony Parker"
    ],
    correctAnswer: "Dirk Nowitzki"
   }
  ],
  nba4: [
   {
    question: "Qual jogador era conhecido por sua tacada “Skyhook”?",
    code: ``,
    answers: [
     "Kareem Abdul-Jabbar",
     "Hakeem Olajuwon",
     "Shaquille O'Neal",
     "Tim Duncan"
    ],
    correctAnswer: "Kareem Abdul-Jabbar"
   },{
    question: "Em qual time Michael Jordan jogou após sua primeira aposentadoria?",
    code: 
``,
    answers: [
     "Washington Wizards",
     "Chicago Bulls",
     "Charlotte Hornets",
     "Houston Rockets"
    ],
    correctAnswer: "Washington Wizards"
   },{
    question: "Qual é o antigo nome da NBA?",
    code: 
``,
    answers: [
     "Liga Americana de Basquete (ABL)",
     "Liga Nacional de Basquete (NBL)",
     "Associação de Basquete da América (BAA)",
     "Associação de Basquete dos Estados Unidos (USBA)"
    ],
    correctAnswer: "Associação de Basquete da América (BAA)"
   },{
    question: "Qual time era originalmente conhecido como New Jersey Nets?",
    code: 
``,
    answers: [
     "Brooklyn Nets",
     "New York Knicks",
     "Philadelphia 76ers",
     "Boston Celtics"
    ],
    correctAnswer: "Brooklyn Nets"
   },{
    question: "Quando foi a primeira aparição do nome NBA?",
    code: 
``,
    answers: [
     "1946",
     "1949",
     "1950",
     "1952"
    ],
    correctAnswer: "1949"
   }
  ],
  nba5: [
   {
    question: "Qual time foi o primeiro a vencer três campeonatos consecutivos da NBA?",
    code: ``,
    answers: [
     "Boston Celtics",
     "Minneapolis Lakers",
     "Chicago Bulls",
     "Los Angelas Clippers"
    ],
    correctAnswer: "Minneapolis Lakers"
   },{
    question: "Quem foi o primeiro jogador da NBA a ter uma média de triplo-duplo em uma temporada?",
    code: ``,
    answers: [
     "Oscar Robertson",
     "Magic Johnson",
     "Russell Westbrook",
     "LeBron James"
    ],
    correctAnswer: "Oscar Robertson"
   },{
    question: "Qual foi o primeiro time da NBA?",
    code: 
``,
    answers: [
     "Boston Celtics",
     "Philadelphia Warriors",
     "Los Angeles Lakers",
     "Toronto Huskies"
    ],
    correctAnswer: "Toronto Huskies"
   },{
    question: "Qual time encerrou a seqüência de oito campeonatos consecutivos da NBA do Boston Celtics em 1967?",
    code: ``,
    answers: [
     "Los Angeles Lakers",
     "Philadelphia 76ers",
     "New York Knicks",
     "Chicago Bulls"
    ],
    correctAnswer: "Philadelphia 76ers"
   },{
    question: "Onde aconteceu o primeiro jogo da NBA?",
    code: ``,
    answers: [
     "Madison Square Garden, Nova York",
     "TD Garden, Boston",
     "Maple Leaf Gardens, Toronto",
     "The Forum, Los Angeles"
    ],
    correctAnswer: "Maple Leaf Gardens, Toronto"
   }
  ],
  nba6: [
   {
    question: "Quanto tempo dura cada quarto de um jogo da NBA?",
    code: ``,
    answers: [
     "10 minutos",
     "20 minutos",
     "15 minutos",
     "12 minutos"
    ],
    correctAnswer: "12 minutos"
   },{
    question: "Quantos jogadores de cada equipe podem entrar na quadra a qualquer momento?",
    code: ``,
    answers: [
     "7",
     "3",
     "5",
     "4"
    ],
    correctAnswer: "5"
   },{
    question: "Qual é o número máximo de faltas pessoais que um jogador pode cometer antes de cometer uma falta em um jogo da NBA?",
    code: ``,
    answers: [
     "4",
     "5",
     "6",
     "7"
    ],
    correctAnswer: "5"
   },{
    question: "Quanto tempo dura o cronômetro de arremessos na NBA?",
    code: ``,
    answers: [
     "20 segundos",
     "24 segundos",
     "30 segundos",
     "35 segundos"
    ],
    correctAnswer: "24 segundos"
   },{
    question: "Qual é a penalidade por falta técnica na NBA?",
    code: ``,
    answers: [
     "Um lance livre e posse de bola",
     "Dois lances livres",
     "Dois lances livres e posse de bola",
     "Um lance livre"
    ],
    correctAnswer: "Um lance livre e posse de bola"
   }
  ],
  nba7: [
   {
    question: "Qual jogador é apelidado de “The Greek Freak”?",
    code: ``,
    answers: [
     "Giannis Antetokounmpo",
     "Nikola Jokić",
     "Luka Doncic",
     "Kristaps Porzingis"
    ],
    correctAnswer: "Giannis Antetokounmpo"
   },{
    question: "Quem ganhou o prêmio NBA MVP em 2000?",
    code: ``,
    answers: [
     "Tim Duncan",
     "Shaquille O'Neal",
     "Allen Iverson",
     "Kevin Garnett"
    ],
    correctAnswer: "Shaquille O'Neal"
   },{
    question: "Quem é o jogador mais jovem a marcar 10,000 pontos na NBA?",
    code: ``,
    answers: [
     "Kobe Bryant",
     "LeBron James",
     "Kevin Durant",
     "Carmelo Anthony"
    ],
    correctAnswer: "LeBron James"
   },{
    question: "Quem ganhou mais campeonatos da NBA como jogador?",
    code: ``,
    answers: [
     "Michael Jordan",
     "Bill Russel",
     "Sam Jones",
     "Tom Heinsohn"
    ],
    correctAnswer: "Bill Russel"
   },{
    question: "Qual jogador foi apelidado de “O Carteiro”?",
    code: ``,
    answers: [
     "Karl Malone",
     "Charles Barkley",
     "Scottie Pippen",
     "Dennis Rodman"
    ],
    correctAnswer: "Karl Malone"
   }
  ],
  nba8: [
   {
    question: "Quem foi o primeiro guard a ser selecionado em primeiro lugar no Draft da NBA?",
    code: ``,
    answers: [
     "Magic Johnson",
     "Allen Iverson",
     "Oscar Robertson",
     "Isiah Thomas"
    ],
    correctAnswer: "Oscar Robertson"
   },{
    question: "Qual jogador tem mais triplos-duplos na carreira na NBA?",
    code: ``,
    answers: [
     "Russell Westbrook",
     "Oscar Robertson",
     "Magic Johnson",
     "LeBron James"
    ],
    correctAnswer: "Russell Westbrook"
   },{
    question: "Quem foi o primeiro jogador a vencer o concurso de três pontos da NBA três vezes?",
    code: ``,
    answers: [
     "Ray Allen",
     "Larry Bird",
     "Steph Curry",
     "Reggie Miller"
    ],
    correctAnswer: "Larry Bird"
   },{
    question: "Qual jogador era conhecido como “Big Fundamental”?",
    code: ``,
    answers: [
     "Tim Duncan",
     "Kevin Garnett",
     "Shaquille O'Neal",
     "Dirk Nowitzki"
    ],
    correctAnswer: "Tim Duncan"
   },{
    question: "Qual jogador detém o recorde da NBA de maior classificação de eficiência de jogador na carreira (PER)",
    code: 
``,
    answers: [
     "LeBron James",
     "Michael Jordan",
     "Shaquille O'Neal",
     "Wilt Chamberlain"
    ],
    correctAnswer: "Michael Jordan"
   }
  ],
  nba9: [
   {
    question: "Quem foi o primeiro jogador a liderar a liga em pontuação e assistências na mesma temporada?",
    code: ``,
    answers: [
     "Oscar Robertson",
     "Nate Archibald",
     "Jerry West",
     "Michael Jordan"
    ],
    correctAnswer: "Nate Archibald"
   },{
    question: "Qual jogador ganhou mais jogos da temporada regular na história da NBA?",
    code: 
``,
    answers: [
     "Kareem Abdul-Jabbar",
     "Robert Parish",
     "Tim Duncan",
     "Karl Malone"
    ],
    correctAnswer: "Kareem Abdul-Jabbar"
   },{
    question: "Quem foi o primeiro jogador da NBA a registrar um quádruplo duplo?",
    code: ``,
    answers: [
     "Hakeem Olajuwon",
     "David Robinson",
     "Nate Thurmond",
     "Alvin Robertson"
    ],
    correctAnswer: "Nate Thurmond"
   },{
    question: "Quem é o único jogador a vencer um campeonato da NBA como jogador-treinador e treinador principal?",
    code: 
``,
    answers: [
     "Bill Russel",
     "Lenny Wilkens",
     "Tom Heinsohn",
     "Bill Sharman"
    ],
    correctAnswer: "Bill Russel"
   },{
    question: "Qual jogador detém o recorde de mais jogos consecutivos disputados na NBA?",
    code:
``,
    answers: [
     "John Stockton",
     "AC Green",
     "Karl Malone",
     "Randy Smith"
    ],
    correctAnswer: "AC Green"
   }
  ],
nba10: [
    {
     question: "Quem é o único jogador que ganhou um título da NBA, uma medalha de ouro olímpica e um campeonato da NCAA?",
     code: ``,
     answers: [
      "Michael Jordan",
      "Magic Johnson",
      "Bill Russel",
      "Larry Bird"
     ],
     correctAnswer: "Michael Jordan"
    },{
     question: "Qual time foi campeão da NBA em 2019-2020?",
     code: 
 ``,
     answers: [
      "Denver Nuggets",
      "Golden State Warriors",
      "Cleveland Cavaliers",
      "Los Angeles Lakers"
     ],
     correctAnswer: "Los Angeles Lakers"
    },{
     question: "Qual foi o time campeão na temporada 1991-1992 da NBA?",
     code: ``,
     answers: [
      "Boston Celtics",
      "San Antonio Spurs",
      "Chicago Bulls",
      "Phoenix Suns"
     ],
     correctAnswer: "Chicago Bulls"
    },{
     question: "Quem foi o MVP da temporada 2021-2022 da NBA?",
     code: 
 ``,
     answers: [
      "Nikola Jokic",
      "Lebron James",
      "Stephen Curry",
      "Joel Embiid"
     ],
     correctAnswer: "Nikola Jokic"
    },{
     question: "Quem foi o defensor do ano na temporada 2019-2020 na NBA?",
     code:
 ``,
     answers: [
      "Andre Drummond",
      "Giannis Antetokounmpo",
      "Kevin Durant",
      "Joel Embiid"
     ],
     correctAnswer: "Giannis Antetokounmpo"
    }
   ],
}
};


// Faça o quiz. Crie um modelo para o estado do nosso aplicativo
function makeQuiz(){
	// Creating an object to store the app's state when beginning the quiz
	return {
		// Gathering a random question out of the available questions for each category
		questions: helpers.getRandomQuestions(STORE),
		// Boolean for if the quiz is in progress or not
		midQuiz: false,
		// Array of correct/incorrect answers to use for our progress bar
		progress: {
			progressBar: [],
			incorrectCategories: []
		},
		// Boolean to determine if the end state should display
		completed: false,
		// Monitoring which question we are currently on
		currentQuestion: 0,
		// Keeps track of total correct answers
		correctAnswers: 0,
		// Keeps current selected answer
		currentAnswer: "",
		// Keeps track of % completed
		percCorrect: 0
	}
}

// Apply fadeOut animations and set the stage for DOM text/element changes
function $fade(appState){

	// This is the completed state of of a quiz
	if(appState.completed){
		
		// Fade out elements with a promise to avoid choppy behavior
		$.when($('.question-answer-wrapper, .question-wrapper, .answer-wrapper').fadeOut(500))
			.done(function(){

				// Display results of the quiz
				$showResults(appState);
				$('.results-wrapper').hide().removeClass('hide');
				$('.question-answer-wrapper, .results-wrapper').fadeIn(500);
	    });

	// This is if the app is just starting
	} else if(appState.midQuiz === false){

		// Set a flag that the app has begun
		appState.midQuiz = true;
		// Fade out elements with a promise to avoid choppy behavior
		$.when($('.question-answer-wrapper, .question-wrapper, .code, .answer-wrapper, .start-quiz, .quit-quiz, .results-wrapper, .progress, .progress-bar').fadeOut(500))
			.done(function(){
				// Remove any progress from a previous quiz (if any);
				helpers.updateProgressBar(appState);
				$('.progress-count').html('1 / 10');
				$('.progress-perc').html('');
				// Kill previous results
				$('.failures').remove();
				// Lots to do... mostly just setting up a new environment for a new quiz
				$updateQuestion(appState);
				$('progress-bar').empty();
				$('progress-fill').html('Progress: <span class="progress-count">1 / 10</span><span class="progress-perc"></span>');
				$('.question-answer-wrapper, .answer-wrapper').removeClass('begin');
				$('.submit-btn, .progress, .progress-bar').removeClass('hide');
				$('.progress, .progress-bar').hide();
				$('.question-answer-wrapper, .question-wrapper, .answer-wrapper, .progress, .progress-bar').fadeIn(500);
				if(!$('.code').hasClass('hide')){$('.code').fadeIn(500)};
	    });

	// This is if the app is in the middle of execution
	} else if(appState.midQuiz){
		$.when($('.question-answer-wrapper, .question-wrapper, .answer-wrapper').fadeOut(500))
			.done(function(){
				$updateQuestion(appState);
				$('.question-answer-wrapper, .question-wrapper, .answer-wrapper').fadeIn(500);
	    });
	}
}

// Work in progress...
function $showResults(appState) {
    let endMsg = '';
    let endFeedback = '';

    appState.percCorrect = parseFloat(appState.correctAnswers / appState.questions.length * 100).toFixed(); // Calcular a porcentagem corretamente

    if (appState.correctAnswers >= 10) {
        endMsg = `Você acertou ${appState.percCorrect}% das perguntas!`;
        endFeedback = 'Parabéns, você é um grande fã de NBA!';
    } else {
        $('.answer-btn').remove();
        endMsg = `Você acertou ${appState.correctAnswers} de ${appState.questions.length} perguntas`;
        endFeedback = 'Muito bem, continue tentando!';
    }

    $('.quiz-end-score').html(endMsg);
    $('.quiz-end-feedback-p').html(endFeedback);
    $('.results-wrapper').removeClass('hide').css('display', 'flex');
    $('.question-answer-wrapper, .results-wrapper').fadeIn(500);
}

// Update the question, code, answers, buttons in the DOM while we're in a faded out state
function $updateQuestion(appState){
	$('.answer-btn').remove();

	// Update the question and code text with the current question
	$('.question').html(appState.questions[appState.currentQuestion].question);

	// This is a hack to hide/show the code portion
	// Only 1/3 of the questions contain code so we hide it if they aren't present
	if(appState.questions[appState.currentQuestion].code == ``){
		$('.code').addClass('hide');
	} else {
		$('.code').removeClass('hide');
		$('.code').html(`<pre>${appState.questions[appState.currentQuestion].code}</pre>`);
	}
	
	// Change continue back to submit
	$('.continue-btn')
		.val("Submit")
		.removeClass('continue-btn')
		.addClass('submit-btn')
		.prop('disabled', true);

	// Add in available answers for the question
	let $answers = [];

	// Adding the answers to a temporary array
	for(let i=0; i<appState.questions[appState.currentQuestion].answers.length; i++){

		// Add current question answers to an array
		let $answer = $('<button class="answer-btn" type="button"></button>');
		$answer.html(appState.questions[appState.currentQuestion].answers[i]);
		$answers.push($answer);
	}

	// Shuffle the answers
	helpers.shuffleAnswers($answers)

	// Push answers to the DOM
	$answers.forEach((answer) => {
		$('.answer-wrapper').prepend(answer);
	});
}

// Simple class and enable/disable DOM selection when answer is selected
function selectAnswer(answer){
	$('.answer-btn').removeClass('selected');
	answer.addClass('selected');
	$('.submit-btn').prop('disabled', false);
}

// Answer is selected and submitted
// Push a feedback state
function submitAnswer(appState){

	// This will be returned true or false based on their answer
	let correct;

	// Add styles to the answers to show if their answer was correct or not
	$('.answer-btn').each(function () {
		if($(this).html() === appState.questions[appState.currentQuestion].correctAnswer){
			$(this).addClass('pass');
			// If answer is correct and selected...
			if($(this).hasClass('selected')){
				correct = "pass";
				appState.correctAnswers++;
				appState.questions[appState.currentQuestion];
			}

		// Handle correct answer if selected answer is incorrect
		} else if ($(this).hasClass('selected')){
			$(this).addClass('fail dim-answer');
			correct = "fail";
			appState.progress.incorrectCategories.push(appState.questions[appState.currentQuestion].category);

		// Dim the other answers to make the correct one more prevalent
		} else {
			$(this).addClass('dim-answer');
		}
	});

	// Add a progress bar indicator to our appState object
	appState.progress.progressBar.push(`<div class="progress-indicator ${correct}"></div>`);
	
	// Update our percent correct (parse a float and set it to a fixed percentage)
	appState.percCorrect = parseFloat(appState.correctAnswers / (appState.currentQuestion + 1) * 100).toFixed();

	// Update our current question VS total quiz length
	$('.progress-count').html(`
		${appState.currentQuestion + 1} / ${appState.questions.length}
	`);

	// Update our current correct percentage
	$('.progress-perc').html(`
		 ◼ ${(appState.percCorrect)}% Correto
	`)

	// Change submit back to continue
	$('.submit-btn')
		.val("Continue")
		.removeClass('submit-btn')
		.addClass('continue-btn')

	// Disable selecting answers
	$('.answer-btn').prop("disabled", true);

	// Update our progress-bar DOM
	helpers.updateProgressBar(appState);

	// Continue to next question
	appState.currentQuestion++;

	// Verify if we're done or not
	if(appState.currentQuestion === appState.questions.length){
		appState.completed = true;
	}
	
}

// Silly easter egg for saying you don't want to do the quiz
function killQuiz(){
	$('.start-quiz, .quit-quiz').hide();
	let failureMsg = "Um dia Jordan disse: 'Eu não posso aceitar não tentar'"
	let msgSplit = failureMsg.split(" ");
	let counter = 0;
	$('.question').empty();
	let startTroll = setInterval(function () {
		$('.question').append(msgSplit[counter] + " ");
		counter++;
		if(counter > msgSplit.length - 1){
  			clearInterval(startTroll);
  			$('.start-quiz').text('Vamos lá!').fadeIn(500);
		}
           }, 250);
}

// These are simple algorithms to modify data that don't need an individual function
let helpers = {
	// Pick a random question from the available ones
	pickRandomQ: function(obj,section){
		return Math.floor(Math.random() * obj.questions[section].length);		
	},
	// Gather a random question from every category
	getRandomQuestions: function(obj){
		let questions = [];
		let categories = Object.keys(obj.questions);
		categories.forEach((cat) => {
			let randomQ = this.pickRandomQ(obj,cat);
			let question = obj.questions[cat][randomQ];
			question.category = cat;
			questions.push(question);
		});
		return questions;
	},
	// Shuffle the answers so they don't appear in the same order
	shuffleAnswers: function(arr){
	    for (var i = arr.length - 1; i > 0; i--) {
	        var j = Math.floor(Math.random() * (i + 1));
	        var temp = arr[i];
	        arr[i] = arr[j];
	        arr[j] = temp;
	    }
	},
	// Update progress bar DOM element
	updateProgressBar: function(appState){
		$('.progress-bar').empty();
		appState.progress.progressBar.forEach((progInd => {
			$('.progress-bar').append(progInd);
		}))
	}
}

// Lets start the show
$(function(){

	// Kill any form refresh
	$('.answer-wrapper').on('submit', function(e){
		e.preventDefault();
	});

	// Storage for quiz app state
	let quizData;

	// Start quiz
	$('.start-quiz, .retry-btn').on('click', function(){
		quizData = makeQuiz();
		$fade(quizData);
	});

	// Select an answer
	$('.question-answer-wrapper').on('click', '.answer-btn', function(){
		selectAnswer($(this));
	})

	// Submit your answer to display feedback and advance question counter
	$('.question-answer-wrapper').on('click', '.submit-btn', function(e){
		submitAnswer(quizData);
	});

	// Submit your answer to display feedback and advance question counter
	$('.question-answer-wrapper').on('click', '.continue-btn', function(){
		$fade(quizData);
	});

	// Easter egg to chastize the user
	$('.quit-quiz').on('click', function(){
		killQuiz();
	})
})